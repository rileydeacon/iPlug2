...
def copy_resources_to_destination(projectpath, dst, label=""):
  ...
  if os.path.exists(projectpath + "/resources/img/"):
    for img in os.listdir(projectpath + "/resources/img/"):
      if img.startswith(".") or img in DONT_COPY:
        continue
  ...
  if os.path.exists(projectpath + "/resources/fonts/"):
    for font in os.listdir(projectpath + "/resources/fonts/"):
      if font.startswith(".") or font in DONT_COPY:
        continue
...
def main():
  ...
  LSMinimumSystemVersion = os.environ.get("MACOSX_DEPLOYMENT_TARGET", xcconfig['DEPLOYMENT_TARGET'])
...
# VST3
  ...
  vst3['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
...
# VST2
  ...
  vst2['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
...
# CLAP
  ...
  clap['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
...
# AUDIOUNIT v2
  ...
  auv2['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
...
#                               NSExtensionServiceRoleType = "NSExtensionServiceRoleTypeEditor",
  ...
  NSExtensionPrincipalClass = "IPlugAUViewController_vtape"
                             )
...
# AAX
  ...
  aax['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
...
# APP
  ...
  macOSapp['CFBundleIdentifier'] = "$(PRODUCT_BUNDLE_IDENTIFIER)"
